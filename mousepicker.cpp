...
// New method from open gl tutorial.
//Somehting happends to the projection and view-matrix from the camera, they do not send the right values.
...
// Old method, not compatible with the open gl tutorial.

//gsl::Vec3 MousePicker::calculateMouseRay()
//{
//    float mouseX = mOwner->mRenderWindow->getMouseX();
//    float mouseY = mOwner->mRenderWindow->getMouseY();
//    gsl::Vec2 normalizedCoords = getNormalizedDeviceCoords(mouseX, mouseY);
//    gsl::Vec4 clipCoords = gsl::Vec4(normalizedCoords.getX(), normalizedCoords.getY(), -1, 1);

//    gsl::Vec4 eyeCoords = toEyeCoords(clipCoords);
//    gsl::Vec3 worldRay= toWorldCoords(eyeCoords);
////    qDebug() << worldRay.getX()<< worldRay.getY()<<worldRay.getZ();
//    return worldRay;
//}

//gsl::Vec4 MousePicker::toEyeCoords(gsl::Vec4 clipCoords)
//{
//    gsl::Mat4 invertedProjection = projectionMatrix;
//    invertedProjection.inverse();
//    gsl::Vec4 eyeCoords = invertedProjection * clipCoords;
//    return gsl::Vec4 (eyeCoords.getX(), eyeCoords.getY(), -1, 1);
//}

//gsl::Vec3 MousePicker::toWorldCoords(gsl::Vec4 eyeCoords)
//{
//    gsl::Mat4 invertedView = viewMatrix;
//    invertedView.inverse();
//    gsl::Vec4 rayWorld = invertedView * eyeCoords;
//    gsl::Vec3 mouseRay = rayWorld.getVec3();
//    mouseRay.normalize();
//    return mouseRay;
//}

//gsl::Vec2 MousePicker::getNormalizedDeviceCoords(float mouseX, float mouseY)
//{
//    float x = (2.f * mouseX) / mOwner->mRenderWindow->width() - 1;
//    float y = (2.f * mouseY) / mOwner->mRenderWindow->height() - 1;
//    return gsl::Vec2(x,y);
//}
